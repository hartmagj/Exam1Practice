...
def problem2a(circle, rectangle, window):
    ...
    lin = rg.Line(rectangle.get_upper_right_corner(), rectangle.get_lower_left_corner())
    lin.arrow = "last"
...
def problem2b(rect, n, delta, win):
    ...
    for k in range(n - 1):
        p1 = rg.Point(rect.get_upper_left_corner().x - (delta * (k + 1)), rect.get_upper_left_corner().y - (delta * (k + 1)))
        p2 = rg.Point(rect.get_lower_right_corner().x + (delta * (k + 1)), rect.get_lower_right_corner().y + (delta * (k + 1)))

        nRect = rg.Rectangle(p1, p2)
        nRect.attach_to(win)
